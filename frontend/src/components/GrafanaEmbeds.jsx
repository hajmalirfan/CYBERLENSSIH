    ...
    <div className="space-y-6">
      <div className="glass-card rounded-2xl p-6 border border-slate-200 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-4">
          ...
          <span className="text-xs font-mono text-emerald-700 flex items-center space-x-1.5 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col justify-between h-72">
            <div className="flex justify-between items-center">
              <span className="text-xs font-bold text-slate-800">7-Day Financial Risk Exposure (₹ Lakhs)</span>
              <span className="text-xs text-slate-400 font-mono">Grafana Panel #101</span>
            </div>
            ...
            <div className="flex items-end space-x-3 h-44 pt-4 px-2">
              {trendDays.map((item, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end group">
                  ...
                  <div
                    className="w-full bg-gradient-to-t from-blue-600 via-indigo-500 to-cyan-400 rounded-t-lg transition-all duration-300 hover:brightness-110"
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 flex flex-col justify-between h-72">
            <div className="flex justify-between items-center">
              <span className="text-xs font-bold text-slate-800">Mean Time to Remediate (MTTR by Tool)</span>
              <span className="text-xs text-slate-400 font-mono">Grafana Panel #102</span>
            </div>
              ...
              {mttrData.map((item, idx) => (
                <div key={idx} className="text-xs">
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="font-mono text-slate-500 font-semibold">{item.hours}</span>
                  </div>
