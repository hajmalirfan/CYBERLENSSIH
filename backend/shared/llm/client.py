...
class SecuriXLLMClient:
    ...
    def __init__(
        ...
        self.ollama_model = (
            ollama_model
            or os.getenv("OLLAMA_MODEL", "qwen2.5:0.5b")
        )
    ...
    async def chat_completion(
        ...
        try:
            async with httpx.AsyncClient(timeout=25.0) as client:
                # Auto-detect available models if chosen model is not present
                try:
                    tags_resp = await client.get(f"{self.ollama_base_url}/api/tags", timeout=3.0)
                    if tags_resp.status_code == 200:
                        avail = [m.get("name", "") for m in tags_resp.json().get("models", [])]
                        if avail and not any(chosen_model in a for a in avail):
            ...
            # Try Ollama's OpenAI-compatible /v1/chat/completions endpoint
            try:
